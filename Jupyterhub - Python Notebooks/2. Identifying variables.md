...
```python
paved_drive = {'Ex':0, 'Gd':1,'TA':2, 'Fa':3, 'Po':4}
def convert_paved_drive(gd):
    return paved_drive[gd]

convert_paved_drive('Gd')
```
...
| KitchenQual | Quality of the kitchen | int | Categorical | 0 | 0-4 (Excellent=0", Good=1", Typical/Average=2", Fair=3", Poor=4") | 1 |
